...
Fan-in is a many-to-one communication pattern for consuming data from many devices through a single data processing and storage channel. 
...
- Transform data values into a ready to use format
  ...
  - _I want to split a string of data into multiple values_
- Aggregate data
...
- _Rules_ are the actions taken on the message
...
1. Devices establish an MQTT connection to the _AWS IoT Core endpoint_, and then publish message to the dt/plant1/dev_n/temp (data telemetry) topic. This is a location and device specific topic to deliver telemetry messages for a given device or sensor.
1. The IoT Rule subscribes to a wildcard topic dt/plant1/*/temp to consolidate messages across devices for the plant and put those messages onto a Kinesis Data Firehose Stream
...
```json
{
    ...
    "sensorData": [
        {
        "sensorName": "Temperature Farenhight",
...
In real world implementations do not embed logic in your lambda function but instead follow good software practices and create a seperate library with unit tests. The lambda is simply the interface to the library maintaining a good seperation of concerns. 